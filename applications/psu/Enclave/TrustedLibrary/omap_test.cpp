...
sgx_status_t ecall_check_union(const uint8_t* p_sealed_buffer, uint32_t sealed_size, const uint8_t* ciphertext, uint8_t* found, uint64_t* value_out){
  uint8_t internal_256bit_val[32] = {0};
  sgx_status_t status = internal_decrypt_rsa_256bit(p_sealed_buffer, ciphertext, internal_256bit_val);
  if (status != SGX_SUCCESS) {
    return status;
  ...
  uint64_t key = convert_256bit_to_uint64(internal_256bit_val);
  printf("[Enclave] Decrypted key for union check: %lu\n", key);
  uint64_t key_cpy = key;
