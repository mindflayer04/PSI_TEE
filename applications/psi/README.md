...
This application implements the Private Set Intersection (PSI) protocol, which allows a client and server to compute the intersection of their private datasets without revealing any additional information. The protocol follows an offline-online computation model to maximize efficiency for unbalanced datasets (where the server's dataset is significantly larger).

### Offline Setup Phase
In the offline phase, the server prepares its dataset to allow for highly efficient online queries:
